...
[PostGraphile](https://www.graphile.org/postgraphile/) in library mode. 
 The entire stack is written in TypeScript, with auto-generated
...
```sh
docker build \
  --file production.server.Dockerfile \
...
Note: If you are using the `production.server.Dockerfile` to run graphile/starter in a
