--! Previous: -
--! Hash: sha1:63a50514a4220f720580d12c52dde288bd806d16
...
/*
 * This function is responsible for reading the `jwt.claims.user_id`
...
/*
 * Logging out deletes the session, and clears the user_id in the
 * transaction. This is a `SECURITY DEFINER` function, so we check that the
 * user is allowed to do it by matching the current_user_id().
 */
...
drop table if exists app_private.user_api_keys cascade;
create table app_private.user_api_keys (
  user_id uuid not null references app_public.users on delete cascade,
  name text,
  api_key_hash text,
  api_key_generated_ts timestamptz
);
alter table app_private.user_api_keys add primary key (user_id, name);
alter table app_private.user_api_keys enable row level security;
grant select, insert, update, delete on app_private.user_api_keys to :DATABASE_VISITOR;

drop table if exists app_public.workspaces cascade;
...
comment on table app_public.workspaces is E'@omit create,update,delete';
alter table app_public.workspaces add primary key (user_id, gitpod_workspace_id);
alter table app_public.workspaces enable row level security;
grant select, insert, update, delete on app_public.workspaces to :DATABASE_VISITOR;
...
create policy manage_as_admin on app_private.user_api_keys for all using (exists (select 1 from app_public.users where is_admin is true and id = app_public.current_user_id()));
